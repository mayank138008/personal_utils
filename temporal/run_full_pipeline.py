...
async def run_full_pipeline(config_path: str) -> str:
    ...
    for idx, prod in enumerate(products[:1]):  # Limiting to 1 product for now
        product_id = prod["id"]
...
#     print(f"🚀 Launching container for product: {product_id}")

#     cmd = [
#         "docker", "run", "--rm",
#         "-v", f"{os.getcwd()}:/data",
#         "-w", "/data",
#         "your-product-image-name:latest",
#         "--config", config_path,
...
#         print(result.stderr)
#         raise RuntimeError(f"❌ Container failed for {product_id}")

#     return f"✅ Finished {product_id}"
import uuid
@activity.defn
def run_product_in_container(product_id: str, config_path: str) -> str:
    """
    Run processing of a single Sentinel product in a Docker container.
    """
    ...
    command = [
        "docker", "run", "--rm",
        "-v", f"{project_root}:/app",               # Mount root project directory
        "-e", f"PRODUCT_ID={product_id}",           # Pass product ID
        "-e", f"CONFIG_PATH={config_path}",         # Pass config path inside container
        "--name", container_name,
        "s2-product-pipeline:latest"                # Image name
    ]
    ...
    try:
        ...
        print(result.stdout)
        return f"✅ {product_id} processed successfully."
    except subprocess.CalledProcessError as e:
        print("❌ Container error:", e.stderr)
        return f"❌ Failed processing {product_id}: {e.stderr}"
