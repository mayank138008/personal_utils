...
async def run_step_in_container(step: str, product_id: str, config_path: str) -> str:
    ...
    # Allow overriding image via env (handy for ECS/K8s)
    image = os.getenv("S2_PIPELINE_IMAGE", "s2-product-pipeline:latest")

    # Mount the config folder into /app/config so CONFIG_PATH is valid in-container
    abs_cfg = (PROJECT_ROOT / config_path).resolve()
    cfg_dir = abs_cfg.parent
    ...
    cmd = [
        ...
        "-e", f"CONFIG_PATH=/app/config/{abs_cfg.name}",
        "-v", f"{cfg_dir}:/app/config",
        image
        # NOTE: no args here; run_one_product.py reads from env
    ]
